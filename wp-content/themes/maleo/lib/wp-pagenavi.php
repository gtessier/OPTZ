...
function wp_pagenavi($before = '', $after = '') {
	...
	if (!is_single()) {
		...
		if($max_page > 1 || intval($pagenavi_options['always_show']) == 1) {
			...
			echo $before.'<ul class="pagination mo-animate" data-animate="bounceIn">'."\n";
			switch(intval($pagenavi_options['style'])) {
				case 1:
					
					if ($start_page >= 2 && $pages_to_show < $max_page) {
						$first_page_text = str_replace("%TOTAL_PAGES%", number_format_i18n($max_page), $pagenavi_options['first_text']);
						echo '<li class="arrow"><a href="'.esc_url(get_pagenum_link()).'" title="'.$first_page_text.'">&laquo;</a></li>';
					...
					if ($end_page < $max_page) {
						...
						echo '<li class="arrow"><a href="'.esc_url(get_pagenum_link($max_page)).'" title="'.$last_page_text.'">&raquo;</a></li>';
					}
					break;				
			}
			echo '</ul>'.$after."\n";
...
function pagenavi_init() {
	...
	$pagenavi_options['next_text'] = __('Next Page','wp-pagenavi');
	$pagenavi_options['prev_text'] = __('Previous Page','wp-pagenavi');
