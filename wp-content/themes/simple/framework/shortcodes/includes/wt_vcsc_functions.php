...
if ( !function_exists( 'WT_VCSC_Excerpt' ) ) {
	function WT_VCSC_Excerpt( $length=30, $readmore, $read_more_text, $post_id='' ) {
		...
		if ( $readmore == true ) {		
			$readmore_link = '<p class="readMore"><a href="'. get_permalink( $id ) .'" title="'.$read_more_text .'" rel="bookmark" class="read_more_link">'.$read_more_text .'</a></p>';
...
if ( !function_exists( 'WT_VCSC_PortfolioPageNavi' ) ) {
	function WT_VCSC_PortfolioPageNavi($before = '', $after = '',$portfolio_query, $paged, $pag_align) {
		...
		if ($max_page > 1 || intval($pagenavi_options['always_show'])) {
			$pages_text = str_replace("%CURRENT_PAGE%", number_format_i18n($paged), $pagenavi_options['pages_text']);
			$pages_text = str_replace("%TOTAL_PAGES%", number_format_i18n($max_page), $pages_text);
			echo balanceTags( $before ) . '<div class="wp-pagenavi"><div class="pagination wt_align_'.$pag_align.'">' . "\n";
			switch(intval($pagenavi_options['style'])){
				// Normal
				case 1:
					if (! empty($pages_text)) {
