...
class Wt_Widget_Tabs_Posts extends WP_Widget {
	...
	function Wt_Widget_Tabs_Posts() {
		$widget_ops = array('classname' => 'widget_tabs_posts', 'description' => esc_html__( "Popular posts, recent posts and comments from your site", 'wt_admin') );
		$this->WP_Widget('tabs_posts', THEME_SLUG.' - '.esc_html__('Tabs Posts', 'wt_admin'), $widget_ops);
...
?>
        ...
        <ul class="wt_tabs"> 
         	<?php if($show_popular): ?>
                <li><a href="#tab-popular" class="current"><?php esc_html_e('Popular','wt_front') ?></a></li>
            <?php endif; ?>
            <?php if($show_recent): ?>
                <li><a href="#tab-recent"><?php esc_html_e('Recent','wt_front') ?></a></li> 
            <?php endif; ?>
        ...
        <div class="wt_clearboth"></div>
           ...
           <div id="tab-comments" class="pane">
    		<ul class="wt_postList">
				<?php
                global $wpdb;
				$comment_approved = 1; // comment is approved
				$comment_type     = '';
				$post_password    = '';
				
                $sql = $wpdb->prepare("SELECT DISTINCT ID, post_title, post_password, comment_ID, comment_post_ID, comment_author, comment_author_email, comment_date_gmt, comment_approved, comment_type, comment_author_url, SUBSTRING(comment_content,1,110) AS com_excerpt FROM $wpdb->comments LEFT OUTER JOIN $wpdb->posts ON ($wpdb->comments.comment_post_ID = $wpdb->posts.ID) WHERE comment_approved = %d AND comment_type = %s AND post_password = %s ORDER BY comment_date_gmt DESC LIMIT $number", $comment_approved,$comment_type,$post_password);
                $the_comments = $wpdb->get_results($sql);
                ...
                <li>
                    ...
                    <div class="wt_postInfo">
                        <p class="wrap"><small class="desc"><em><?php echo strip_tags($comment->comment_author); ?></em><?php esc_html_e('says:', 'wt_front'); ?>:</small></p>
                        <div class="meta">
                            <a class="comment-text-side" href="<?php echo get_permalink($comment->ID); ?>#comment-<?php echo (int)$comment->comment_ID; ?>" title="<?php echo strip_tags($comment->comment_author); ?> on <?php echo esc_attr( $comment->post_title ); ?>"><?php echo string_limit_words(strip_tags($comment->com_excerpt), 8); ?>...</a>
                        </div>
                    </div>
                </li>
                <?php } ?>
...
?>
        ...
        <p><input type="checkbox" class="checkbox" id="<?php echo esc_attr( $this->get_field_id('show_popular') ); ?>" name="<?php echo esc_attr( $this->get_field_name('show_popular') ); ?>"<?php checked( $show_popular ); ?> />
		<label for="<?php echo esc_attr( $this->get_field_id('show_popular') ); ?>"><?php esc_html_e( 'Show popular posts?' , 'wt_admin'); ?></label></p>

<p><input type="checkbox" class="checkbox" id="<?php echo esc_attr( $this->get_field_id('show_recent') ); ?>" name="<?php echo esc_attr( $this->get_field_name('show_recent') ); ?>"<?php checked( $show_recent ); ?> />
		<label for="<?php echo esc_attr( $this->get_field_id('show_recent') ); ?>"><?php esc_html_e( 'Show recent posts?' , 'wt_admin'); ?></label></p>

<p><input type="checkbox" class="checkbox" id="<?php echo esc_attr( $this->get_field_id('show_comments') ); ?>" name="<?php echo esc_attr( $this->get_field_name('show_comments') ); ?>"<?php checked( $show_comments ); ?> />
		<label for="<?php echo esc_attr( $this->get_field_id('show_comments') ); ?>"><?php esc_html_e( 'Show comments?' , 'wt_admin'); ?></label></p>
