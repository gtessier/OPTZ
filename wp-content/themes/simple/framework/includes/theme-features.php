...
class wt_themeFeatures {
	...
	function wt_section($post_id = NULL){
		...
		while($r->have_posts()) {
			...
			$section_border_top = get_post_meta($ids, '_border_style_color', true);
			...
			$top_margins = get_post_meta($ids, '_top_margins', true);
			$bottom_margins = get_post_meta($ids, '_bottom_margins', true);
			...
			if(!empty($section_border_top) && $section_border_top != "transparent"){
				$section_border_top = 'border:1px solid '.$section_border_top.';';
			}else{
				$section_border_top = '';
			}
			...
			if($bgType == 'parallax') {
				echo '<section id="'. get_the_slug($post_id) .'_separator" class="wt_section_area wt_parallax '. $section_color .'"';
				if(!empty($section_parallax_custom)) {echo ' style="'. $top_margins . $bottom_margins . $section_parallax_custom.'"';
				} 			
				echo '>';
				...
				echo '<section id="'. get_the_slug($post_id) .'_separator" class="wt_section_area '. $section_color .'"';
				if(!empty($section_bg_color) || !empty($section_border_top)) {echo ' style="'. $top_margins . $bottom_margins . $section_bg_color.$section_border_top.'"';		
				}
				echo '>';
				if(!empty($section_bg_overlay)) {echo ' 
					<div class="wt_section_overlay" style="'.$section_bg_overlay.'"></div>';
				...
				echo '<section id="'. get_the_slug($post_id) .'_separator" class="wt_section_area'. $section_color .'"';
				if(!empty($section_img_cover) || !empty($section_border_top)) {echo ' style="background-image:url('.$section_img_cover.');background-size: cover; background-attachment: fixed;'. $top_margins . $bottom_margins . $section_border_top.'"';		
				}
				echo '>';
				if(!empty($section_bg_overlay)) {echo ' 
					<div class="wt_section_overlay" style="'.$section_bg_overlay.'"></div>';
				...
				echo '<section id="'. get_the_slug($post_id) .'_separator" class="wt_section_area wt_pattern '. $section_color .'"';
				if(!empty($section_color_custom) || !empty($section_img_custom) || !empty($section_border_top)) {echo' style="'. $top_margins . $bottom_margins . $section_img_custom.$section_border_top.'"';}
				echo '>';
				if(!empty($section_bg_overlay)) {echo ' 
					<div class="wt_section_overlay" style="'.$section_bg_overlay.'"></div>';
				} 
				...
				echo '<section id="'. get_the_slug($post_id) .'_separator" class="color wt_section_area'. $section_color .'"';
				if(!empty($section_bg_color) || !empty($section_img_custom) || !empty($section_border_top)) {echo ' style="background-color:'. $top_margins . $bottom_margins . $section_bg_color.$section_border_top.';"';		
				}
				echo '>'; 
				if(!empty($section_bg_overlay)) {echo ' 
					<div class="wt_section_overlay" style="'.$section_bg_overlay.'"></div>';
	...
	function wt_nav($post_id = NULL) {
		...
		/*if(!wt_get_option('general','display_logo') && $custom_logo = wt_get_option('general','logo')){ 		
                   echo '<div id="logo">';
            	if($enable_retina){
                echo '<a href="'. esc_url( home_url( '/' ) ) .'"><img src="'. $custom_logo .'" data-at2x="'. $retinaLogo .'" /></a>';
				} else {
	...
	function wt_custom_header($post_id = NULL) {
		...
		if (isset($title)) {
			...
			echo '<div class="wt_separator_title wt_animate wt_animate_if_visible" data-animation="bounceIn"><div class="wt_separator_first_circle"><div class="wt_separator_second_circle"></div></div></div>';
		}
		if (isset($text)) {
	...
	function wt_section_title() {
		$type = get_post_meta(get_the_ID(), '_intro_type', true);
			...
			if (in_array($type, array('custom', 'title_custom'))) {
				...
				$text = '<div class="wt_separator_title wt_animate wt_animate_if_visible" data-animation="bounceIn"><div class="wt_separator_first_circle"><div class="wt_separator_second_circle"></div></div></div>';
			}
		//echo wt_theme_generator('wt_intro',$post_id);		
